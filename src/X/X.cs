...
public class MegamanX : Character {
	...
	public override void preUpdate() {
		...
		if (stockedLv1Charge || stockedX2Charge || stockedX3Saber || stockedX3Charge) {
	...
	public override bool attackCtrl() {
		...
		if (bufferedShotPressed && stockedX3Charge) {
			shoot(1, specialBuster, false);
			return true;
		}
		if (bufferedShotPressed && stockedX2Charge) {
			shoot(1, currentWeapon ?? specialBuster, true);
			return true;
		}
		if (currentWeapon != null && canShoot() && (
				(!isCharging() && !currentWeapon.isStream &&
	...
	public void shoot(int chargeLevel, Weapon weapon, bool busterStock) {
		...
		if (chargeLevel >= 3 && armArmor == ArmorId.Giga || busterStock) {
			if (!weapon.hasCustomChargeAnim && charState is not WallSlide) useCrossShotAnim = true;
			if (!busterStock) {
		...
		if (!stockedX2Charge || busterStock || weapon.fireRate <= 10) {
	...
	public void maxArmorChargeShots(int type, HyperCharge hcWep) {
		...
		if (type == 0) {
			new BusterX3Proj1(shootPos, shootDir, this, player, player.getNextActorNetId(), rpc: true
			);
			...
			new Buster2Proj(pos, xDir, this, player, player.getNextActorNetId(), true);
			playSound("buster3X3", sendRpc: true);
			stockLv1Charge(false);
			shootCooldown = 30f;
		} else if (type == 3) {
			new Buster3MaxProj(pos, xDir, this, player, player.getNextActorNetId(), true);
			playSound("buster3X3", sendRpc: true);
			stockX3Charge(false);
			shootCooldown = 30f;
		}
	...
	public override bool canShoot() {
		if (isInvulnerableAttack() ||
			hasLastingProj() && !stockedX2Charge && !stockedX3Charge && !stockedX3Saber ||
			hasLockingProj() ||
			shootCooldown > 0 ||
			invulnTime > 0 ||
	...
	public override void updateCustomActorNetData(byte[] data) {
		...
		stockedX2Charge = boolData[1];
		stockedX3Charge = boolData[2];
		stockedX3Saber = boolData[3];
		 ...
		 charState is not LadderClimb && player.weapon != null && aiAttackCooldown <= 0) {
			switch (Xattack) {
				...
				case 6 when player.hasArmArmor(3):
					...
					break;
				case 7 when stockedX2Charge || stockedX3Charge || stockedX3Saber:
					player.press(Control.Shoot);
					player.release(Control.Shoot);
					break;
				default:
