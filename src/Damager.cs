		...
		if (character != null) {
			...
			switch (projId) {
				...
				case (int)ProjIds.FireWave:
					character.addBurnTime(owner, new FireWave(), 0.75f);
					break;
				...
				case (int)ProjIds.Ryuenjin:
					character.addBurnTime(owner, RyuenjinWeapon.staticWeapon, 1.5f);
					break;
				...
				case (int)ProjIds.DrDopplerDash:
					character.addBurnTime(owner, new Weapon(WeaponIds.DrDopplerGeneric, 156), 2f);
					break;
				...
				case (int)ProjIds.BBuffaloBeam:
					character.addIgFreezeProgress(4, 4 * 60);
					break;
